...
class InvoiceViewSet(viewsets.ModelViewSet):
    ...
    def summary(self, request, pk=None):
        ...
        try:
            import httpx
            lines = [
                'Summarize this freight invoice in 2-3 sentences for an operations manager.',
                'Be concise and highlight any risks.',
                '',
                'Invoice: ' + str(invoice.invoice_number),
                'Vendor: ' + str(invoice.vendor_name),
                'Amount: $' + f'{float(invoice.total_amount):,.2f}',
                'Confidence: ' + str(round((invoice.confidence_score or 0) * 100)) + '%',
                'Status: ' + str(invoice.validation_status),
                'Discrepancies: ' + str(invoice.discrepancy_count),
                'Notes: ' + str(invoice.validation_notes or 'none'),
            ]
            prompt = '\n'.join(lines)
            res = httpx.post(
            ...
            return Response({'summary': 'Summary unavailable: ' + str(e)})
